...
// ------------------------------------------------------------------------------------------
   ...
   if(wFORMS) {
		...
		wFORMS.behaviors['validation'] = {
			...
			errMsg_alpha 		: "Do not enter numbers in text fields (a-z). ",
			errMsg_email 		: "The email address is invalid. ",
			errMsg_dni 			: "The DNI is invalid (eg. 12345678H or X1234567R). ",
			errMsg_integer 		: "Enter an integer (eg. 3).",
			errMsg_float 		: "Enter a decimal number (eg. 1.9).",
			errMsg_password 	: "The password is not secure. Enter four to twelve characters and combine them sensitively.", // password
			errMsg_alphanum 	: "Enter only alphanumeric characters (az and 0-9). ",
			errMsg_date 		: "The date is not correct.",
			errMsg_notification : "%% error (s) detected. The form could not be sent. \n Verify that the data entered.", // %% errors.
			errMsg_custom		: "Enter a valid value.",
           ...
           run: function(e) {
						...
						for (j=0;j<arrClasses.length;j++) {
							switch(arrClasses[j]) {
								...
								case "validate-email":
									...
									break;
								case "validate-dni":
									if(!wBehavior.isDni(element.value)) {
										wBehavior.showError(element, wBehavior.errMsg_dni);
										nbErrors++;
									}
									break;
			...
			isAlpha: function(s) {
				//var regexpAlphabetic = /^[a-zA-Z\s]+$/; // Add ' and - ?  //modificat per al catala
				var regexpAlphabetic = /^[\u0041-\u007A\u00C0-\u00FF\u0100-\u017F]+$/; 
				return wFORMS.behaviors['validation'].isEmpty(s) || regexpAlphabetic.test(s);
			},
			isAlphaNum: function(s) {
				//var validChars = /^[\w\s]+$/; //modificat per al catala
				var validChars = /^[\u0030-\u0039\u0041-\u007A\u00C0-\u00FF\u0100-\u017F]+$/;
			...
			isDni: function(s) {
				var regexpDni = /[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][A-Z]/;
				var regexpDni2 = /[A-Z][0-9][0-9][0-9][0-9][0-9][0-9][0-9][A-Z]/;
				return wFORMS.behaviors['validation'].isEmpty(s) || regexpDni.test(s) || regexpDni2.test(s);
			},
